        ...
        let payementProcess =
            <Form action="/simulator">
                ...
                <InputGroup className="text-left" size="lg">
                    ...
                    <Input onChange={(e) => this.inputChangeHandler(e, "amount")} name="amount" required style={{ height: '65px' }} disabled value={this.props.price} type="number" className="bg-dark border-0 text-light" placeholder="Amount to pay" />
                </InputGroup>
                <br />
                <FormGroup row>
...
const mapStateToProps = state => {
    ...
    return {
        paymentLink: state.payement.paymentlink,
        price: state.payement.price
    };
};
